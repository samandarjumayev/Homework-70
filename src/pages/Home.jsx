    ...
    return <div className="flex-1 flex flex-col gap-3">
        <div className="flex-1 grid grid-cols-4 grid-rows-2 gap-4">
            {data?.articles?.slice(startPage, endPage).map((item, index) => {
                return <div key={index} className="h-[207px] bg-white border rounded overflow-hidden shadow-md shadow-zinc-400 cursor-pointer transition-all duration-200 hover:duration-150 hover:scale-107">
        ...
        <div className=" flex justify-center items-center gap-1">
            <p onClick={() => {
                setStartPage(0);
                setEndPage(8);
            }} className={'hover:bg-[#109BE9] hover:text-white px-[7px] rounded-[3px] cursor-pointer'}>1</p>

            <p onClick={() => {
                setStartPage(9);
                setEndPage(17)
            }} className={'hover:bg-[#109BE9] hover:text-white px-[7px] rounded-[3px] cursor-pointer'}>2</p>
        </div>
    </div>
}